...
public class NotificationManager {
	...
	public void initialize() {
		...
		String password;
		try {
			propsMap = commonUtils.getMapFromPropertyFile(CarrierConstants.NOTIFICATION_PROPS_FILE);
		} catch (CarrierClientException e) {
			logger.error("Unable to load Notification properties : " + CarrierConstants.NOTIFICATION_PROPS_FILE);
		...
		if (null != propsMap){
			...
			props.put(CarrierConstants.SMTP_HOST, propsMap.get(CarrierConstants.SMTP_HOST));
			props.put(CarrierConstants.SMTP_PORT, propsMap.get(CarrierConstants.SMTP_PORT));
			
			authenticated = propsMap.get(CarrierConstants.SMTP_AUTHENTICATE);
			userName = propsMap.get(CarrierConstants.SMTP_USERNAME);
			password = propsMap.get(CarrierConstants.SMTP_PD);
			
			Protocol protocol = Protocol.valueOf(propsMap.get(CarrierConstants.SMTP_PROTOCOL));
			switch(protocol){
			 case SMTPS:
	...
	public void sendNotification(String message, String recipients ,String subject) {
		...
		if (null != propsMap && null != session){
			...
			fromAddress = propsMap.get(CarrierConstants.MAIL_FROM_ADDRESS);		
			
			if (null == recipients){
				receipientList = propsMap.get(CarrierConstants.MAIL_TO_ADDRESS_LIST);
			}
				...
				try {
					...
					logger.error("Unable to send Email Notification -" + mesgEx);
